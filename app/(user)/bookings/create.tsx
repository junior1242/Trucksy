...
const [requiredVehicleType, setRequiredVehicleType] = useState<VehicleType>('TRUCK');
...
return j?.features?.[0]?.place_name || `${lat.toFixed(5)}, ${lng.toFixed(5)}`;
...
const r = await fetch(url); const j = await r.json();
setResults((j?.features||[]).map((f:any)=>({ id:f.id, place_name:f.place_name, center:f.center })));
setShowResults(true);
} finally { setSearching(false); }
};
const onChangeQuery = (text:string) => { setQuery(text); if(debounceTimer.current) clearTimeout(debounceTimer.current); debounceTimer.current = setTimeout(()=>runSearch(text), 350); };
...
return (
<View style={{ flex: 1 }}>
<MapboxGL.MapView style={{ flex:1 }} logoEnabled={false} compassEnabled onRegionDidChange={onRegionDidChange}>
<MapboxGL.Camera ref={cameraRef} zoomLevel={13} centerCoordinate={center} animationMode="easeTo" animationDuration={300} />
{pickup && dropoff && (
<MapboxGL.ShapeSource id="route" shape={{ type:'Feature', geometry:{ type:'LineString', coordinates:[pickup, dropoff] }, properties:{} }}>
...
<View style={{ backgroundColor: phase==='pickup' ? '#2ecc71' : '#e74c3c', width:18, height:18, borderRadius:9, borderWidth:2, borderColor:'white', shadowColor:'#000', shadowOpacity:0.2, shadowRadius:6, elevation:3 }} />
    </View>
...
{/* Address chips */}
<View style={{ position:'absolute', top:160, left:12, right:12, gap:8 }}>
<View style={{ backgroundColor:'white', borderRadius:10, padding:10, borderLeftWidth:4, borderLeftColor:'#2ecc71' }}>
<Text style={{ fontSize:12, color:'#2ecc71' }}>Pickup</Text>
<Text numberOfLines={1}>{pickup ? pickupLabel : (phase==='pickup' ? 'Move map or search…' : pickupLabel)}</Text>
</View>
<View style={{ backgroundColor:'white', borderRadius:10, padding:10, borderLeftWidth:4, borderLeftColor:'#e74c3c' }}>
<Text style={{ fontSize:12, color:'#e74c3c' }}>Dropoff</Text>
<Text numberOfLines={1}>{dropoff ? dropoffLabel : (phase==='dropoff' ? 'Move map or search…' : dropoffLabel)}</Text>
</View>
</View>
...
return (
<TouchableOpacity key={vt} onPress={()=>setRequiredVehicleType(vt)} style={{ backgroundColor: selected ? '#111' : '#f0f0f0', borderRadius:999, paddingVertical:8, paddingHorizontal:14 }}>
<Text style={{ color: selected ? 'white' : '#111' }}>{vt.replace('_',' ')}</Text>
</TouchableOpacity>
);
})}
</ScrollView>
...
return (
<TouchableOpacity key={lt} onPress={()=>setLoadType(lt)} style={{ backgroundColor: selected ? '#111' : '#f0f0f0', borderRadius:999, paddingVertical:8, paddingHorizontal:14 }}>
...
{/* Bottom bar */}
<View style={{ position:'absolute', left:0, right:0, bottom:0, backgroundColor:'white', padding:12, borderTopWidth:1, borderColor:'#eee' }}>
<View style={{ flexDirection:'row', justifyContent:'space-between', marginBottom:8 }}>
<TouchableOpacity onPress={swap} disabled={!pickup || !dropoff} style={{ opacity:!pickup||!dropoff?0.4:1, backgroundColor:'#f0f0f0', paddingVertical:8, paddingHorizontal:14, borderRadius:8 }}><Text>Swap</Text></TouchableOpacity>
<TouchableOpacity onPress={resetAll} style={{ backgroundColor:'#f0f0f0', paddingVertical:8, paddingHorizontal:14, borderRadius:8 }}><Text>Reset</Text></TouchableOpacity>
</View>
{phase==='pickup' ? (
